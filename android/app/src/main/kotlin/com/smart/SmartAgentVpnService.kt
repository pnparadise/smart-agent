...
class SmartAgentVpnService : VpnService() {
    ...
    private fun startVpn(fileName: String) {
        ...
        coroutineScope.launch(Dispatchers.IO) {
            try {
                ...
                val baseConfig = Config.parse(ByteArrayInputStream(tunnelContent.toByteArray()))
                val configWithDoh = applyDohConfig(baseConfig)
    ...
    private suspend fun applyDohConfig(originalConfig: Config): Config {
        val dohConfig = SmartConfigRepository.dohConfig.value
        val dohUrl = dohConfig.dohUrl.trim()
        if (!dohConfig.enabled || dohUrl.isEmpty()) return originalConfig

        return try {
            val resolvedConfig = WgConfigMultiResolver.resolveAllEndpoints(originalConfig, dohUrl)
            val finalText = resolvedConfig.toWgQuickString()
            SmartConfigRepository.logDebug(
                "Config Ready with DoH ($dohUrl):\n$finalText",
                tag = "WireGuard",
                level = "INFO"
            )
            resolvedConfig
        } catch (e: Exception) {
            SmartConfigRepository.logDebug("DoH resolve failed: ${e.message}", tag = "SmartAgent", level = "ERROR")
            originalConfig
        }
    }
